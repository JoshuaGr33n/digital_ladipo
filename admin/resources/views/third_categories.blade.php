        ...
        @extends('layout')
        @section('content') 
        @section('title', $secondCat->second_category)
...
<?php 
// $back_link = DB::select("SELECT main_category FROM categories WHERE id = 6");
...
<div   style="margin:auto;" width="40%">
    <div class="element-wrapper">
        <h6 class="element-header">{{ucwords($secondCat->second_category)}}<a href="{{url(str_replace(' ','-',$mainCat->main_category))}}" class="mr-2 mb-2 btn btn-warning"  style="margin-left:500px">Back</a></h6>
        <div class="element-box">
            <h5 class="form-header">Categories/{{$mainCat->main_category}}/{{$secondCat->second_category}}<button class="mr-2 mb-2 btn btn-success" type="button" style="margin-left:300px" data-toggle="modal" data-target="#ModalAdd">Add Category+</button></h5>
            <div class="form-desc">
            ...
            <div class="table-responsive">
                <table id="dataTable1" width="100%" class="table table-striped table-lightfont">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{ ++$i }}</td>
                            <td>{{ ucwords($category->third_category) }} </td>
                          
                          <td>

                                 <form action="{{$secondCat->second_category}}/update_third_category" method="POST">
                                   {{ csrf_field() }}
                                   {{ method_field('PATCH') }}
                                  ...
                                  @include('modals.edit_third_category_modal')
                                  </form>
                                  </td>
                                  <td> 
                                    @php 
                                  ...
                                  <form action="{{$secondCat->second_category}}/delete_third_category" method="POST">
                                  {{-- @csrf
                                   @method('DELETE') --}} 
            ...
            <div class="modal-body">
             ...
             @if ($errors->any())
              ...
              <ul>
               ...
               @endif
                <form  method="POST" action="{{$mainCat->main_category}}{{$secondCat->main_category}}/post"  class="btn-submit" id="formValidate">
                {{ csrf_field() }}
